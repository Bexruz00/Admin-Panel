  ...
  return (
    <form onSubmit={handleAddGroup} autoComplete="off" className="p-5">
      ...
      <CrudCaption
        id={groupId}
        isLoading={isLoading}
        title={`Gurux ${groupId ? "tahrirlash" : "qo'shish"}`}
      />
      <div className="flex justify-evenly mt-10">
        <div className="w-[40%] flex flex-col gap-5">
          ...
          />
          <FilterCustom
            API={`/teachers?stackId=${stackId}`}
            filterId={teacherId}
            setFilterId={setTeacherId}
            setFilterName={setMainTeacher}
          ...
          <Input
            ...
            required
            size="large"
            placeholder="Yordamchi ustozning ismi"
          />
          <FilterCustom
            API={"/rooms"}
            filterId={roomId}
            setFilterId={setRoomId}
            setFilterName={setRoom}
            placeholder={"Xona tanlang"}
            extraClass={"!w-full"}
          />
          <DatePicker
            onChange={(a, b) => setCreateAt(b)}
            className="w-full"
            size="large"
            defaultValue={dayjs(createdAt, dateFormat)}
          />
        </div>
      </div>
    </form>
